...
function SignaturePad({ title, signerName, value, onChange }) {
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white p-3">
    <div className="mb-2 flex items-center gap-2"><div className="mr-auto"><b className="text-sm text-slate-800">{title}</b><div className="text-xs text-slate-500">{signerName}</div></div><button type="button" onClick={clear} className="flex items-center gap-1 rounded-lg border px-2 py-1 text-xs text-slate-500"><Eraser size={13} /> Xóa chữ ký</button></div>
    ...
    <div className={`mt-1 text-xs ${value ? "text-emerald-600" : "text-amber-600"}`}>{value ? "Đã ghi nhận chữ ký" : "Có thể để trống hoặc ký bằng chuột / màn hình cảm ứng"}</div>
  </div>;
}
...
export default function AssetHandoverModal({ handover, saving, onComplete, onCancel }) {
  ...
  const companyName = snapshot.company?.name || snapshot.company?.code || "CÔNG TY";
    ...
    <div className="mx-auto w-full max-w-6xl overflow-hidden rounded-3xl bg-slate-100 shadow-2xl">
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-[1.25fr_.75fr]">
        <article className="mx-auto min-h-[760px] w-full max-w-[794px] bg-white px-8 py-9 font-serif text-[15px] leading-relaxed text-slate-900 shadow-sm sm:px-14">
          <div className="grid grid-cols-2 text-center text-sm leading-tight"><b className="uppercase">{companyName}</b><b>CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM<br /><u>Độc lập - Tự do - Hạnh phúc</u></b></div>
          ...
          <h2 className="mt-5 font-bold uppercase">1. {isReturn ? "Người giao lại tài sản" : "Người giao"}</h2><p>Họ và tên: <b>{giverName || "-"}</b></p>{isReturn ? <p>Mã nhân viên: {employee.employeeCode} · Bộ phận: {employee.department || "-"} · Chức danh: {employee.jobTitle || "-"}</p> : <p>Đơn vị: {companyName}</p>}
          <h2 className="mt-4 font-bold uppercase">2. {isReturn ? "Người nhận thu hồi" : "Người nhận"}</h2><p>Họ và tên: <b>{receiverName || "-"}</b></p>{isReturn ? <p>Đơn vị: {companyName}</p> : <p>Mã nhân viên: {employee.employeeCode} · Bộ phận: {employee.department || "-"} · Chức danh: {employee.jobTitle || "-"}</p>}
        ...
        <aside className="space-y-3 lg:sticky lg:top-24 lg:self-start"><SignaturePad title={isReturn ? "NGƯỜI GIAO LẠI" : "NGƯỜI GIAO"} signerName={giverName} value={giverSignature} onChange={setGiverSignature} /><SignaturePad title={isReturn ? "NGƯỜI NHẬN THU HỒI" : "NGƯỜI NHẬN"} signerName={receiverName} value={receiverSignature} onChange={setReceiverSignature} />
          ...
          <div className="rounded-xl bg-amber-50 p-3 text-xs text-amber-800">Có thể bỏ trống một hoặc cả hai ô chữ ký. Khi hoàn tất, hệ thống tạo PDF, lưu lên Google Drive và chính thức {isReturn ? "thu hồi thiết bị theo phương án đã chọn" : "chuyển thiết bị sang trạng thái đang cấp phát"}.</div>
          <div className="flex justify-end gap-2"><button disabled={saving} onClick={onCancel} className="rounded-xl border bg-white px-4 py-2 text-sm">Hủy biên bản</button><button disabled={saving || !accepted} onClick={() => onComplete({ giverSignature, receiverSignature })} className={`flex items-center gap-2 rounded-xl px-5 py-2 text-sm font-bold text-white disabled:opacity-50 ${isReturn ? "bg-orange-600" : "bg-blue-600"}`}>{saving ? <Loader2 size={16} className="animate-spin" /> : <FileSignature size={16} />}{saving ? "Đang tạo và lưu PDF..." : `Hoàn tất ${isReturn ? "thu hồi" : "bàn giao"}`}</button></div>
        </aside>
      </div>
    </div>
  </div>;
