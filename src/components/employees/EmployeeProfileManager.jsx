  ...
  return <>
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/50 p-4"><div className="max-h-[94vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
      ...
      <div className="mb-5 rounded-2xl border border-violet-200 bg-violet-50 p-4">
        <label><span className={labelClass}>Mẫu hợp đồng theo vị trí</span><select value={draft.templateId || ""} disabled={Boolean(draft._id && draft.status !== "draft")} onChange={(event) => onApplyTemplate(event.target.value)} className={`${inputClass} disabled:bg-slate-100`}><option value="">Không dùng mẫu / nhập thủ công</option>{draft.templateId && !activeTemplates.some((item) => item._id === draft.templateId) && <option value={draft.templateId}>{draft.templateName || draft.templateCode} · v{draft.templateVersion}</option>}{activeTemplates.map((item) => <option key={item._id} value={item._id}>{item._suggested ? "★ Gợi ý · " : ""}{item.name} · v{item.version}{item.sourceDocx?.originalName ? " · Word gốc" : ""}</option>)}</select></label>
        <p className="mt-2 text-xs text-violet-700">Mẫu được gợi ý từ bộ phận “{department || "chưa xác định"}” và chức danh “{jobTitle || "chưa xác định"}”. Chỉ hợp đồng nháp mới được đổi hoặc áp dụng lại mẫu.</p>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <Field label="Lương cơ bản" type="number" value={draft.baseSalary} onChange={(v) => updateField("baseSalary", Number(v))} />
