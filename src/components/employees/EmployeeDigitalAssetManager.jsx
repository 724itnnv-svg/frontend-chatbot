...
import { Download, ExternalLink, History, KeyRound, Pencil, Plus, RefreshCcw, RotateCcw, Save, Search, ShieldCheck, Trash2, Upload, UserPlus, X } from "lucide-react";
...
const DIGITAL_IMPORT_HEADERS = ["MÃ TÀI SẢN SỐ", "LOẠI", "TÊN TÀI SẢN", "NHÀ CUNG CẤP", "TÀI KHOẢN/ĐỊNH DANH", "ĐƯỜNG DẪN ĐĂNG NHẬP", "EMAIL KHÔI PHỤC", "SĐT KHÔI PHỤC", "ĐÃ BẬT 2FA", "NƠI LƯU XÁC THỰC", "NGÀY GIA HẠN", "CHI PHÍ/THÁNG", "GHI CHÚ", "MÃ NHÂN VIÊN ĐANG GIỮ", "NGÀY CẤP PHÁT"];
const normalizeImportText = (value) => String(value ?? "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/Đ/g, "D").replace(/đ/g, "d").toLowerCase().replace(/\s+/g, " ").trim();
const importCell = (row, names) => {
  const targets = names.map(normalizeImportText);
  const key = Object.keys(row || {}).find((item) => targets.includes(normalizeImportText(item)));
  return key ? row[key] : "";
};
...
function parseDigitalImportRow(row, index) {
  const mfa = normalizeImportText(importCell(row, ["ĐÃ BẬT 2FA", "MFA", "mfaEnabled"]));
  return {
    rowNumber: index + 2,
    assetCode: String(importCell(row, ["MÃ TÀI SẢN SỐ", "MÃ TÀI SẢN", "assetCode"]) || "").trim().toUpperCase(),
    category: String(importCell(row, ["LOẠI", "category"]) || "other").trim().toLowerCase(),
    name: String(importCell(row, ["TÊN TÀI SẢN", "TÊN", "name"]) || "").trim(),
    provider: String(importCell(row, ["NHÀ CUNG CẤP", "provider"]) || "").trim(),
    accountIdentifier: String(importCell(row, ["TÀI KHOẢN/ĐỊNH DANH", "TÀI KHOẢN", "accountIdentifier"]) || "").trim(),
    loginUrl: String(importCell(row, ["ĐƯỜNG DẪN ĐĂNG NHẬP", "loginUrl"]) || "").trim(),
    recoveryEmail: String(importCell(row, ["EMAIL KHÔI PHỤC", "recoveryEmail"]) || "").trim(),
    recoveryPhone: String(importCell(row, ["SĐT KHÔI PHỤC", "recoveryPhone"]) || "").trim(),
    mfaEnabled: ["co", "yes", "true", "1", "da bat"].includes(mfa),
    credentialLocation: String(importCell(row, ["NƠI LƯU XÁC THỰC", "credentialLocation"]) || "").trim(),
    renewalDate: importDate(importCell(row, ["NGÀY GIA HẠN", "renewalDate"])),
    monthlyCost: Number(String(importCell(row, ["CHI PHÍ/THÁNG", "monthlyCost"]) || "").replace(/[^\d.-]/g, "")) || 0,
    notes: String(importCell(row, ["GHI CHÚ", "notes"]) || "").trim(),
    employeeCode: String(importCell(row, ["MÃ NHÂN VIÊN ĐANG GIỮ", "MÃ NHÂN VIÊN", "employeeCode"]) || "").trim().toUpperCase(),
    assignedAt: importDate(importCell(row, ["NGÀY CẤP PHÁT", "NGÀY GIAO", "assignedAt"])),
  };
}
...
export default function EmployeeDigitalAssetManager({ standalone = false }) {
  ...
  const downloadImportTemplate = () => {
    const sample = Object.fromEntries(DIGITAL_IMPORT_HEADERS.map((header) => [header, ""]));
    Object.assign(sample, { "MÃ TÀI SẢN SỐ": "MAIL-001", "LOẠI": "gmail", "TÊN TÀI SẢN": "Email công việc", "NHÀ CUNG CẤP": "Google", "TÀI KHOẢN/ĐỊNH DANH": "nhanvien@congty.com", "ĐÃ BẬT 2FA": "Có" });
    const sheet = XLSX.utils.json_to_sheet([sample], { header: DIGITAL_IMPORT_HEADERS });
    sheet["!cols"] = DIGITAL_IMPORT_HEADERS.map(() => ({ wch: 24 }));
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, sheet, "Tai san so");
    XLSX.writeFile(workbook, "mau-import-tai-san-so.xlsx");
  };
