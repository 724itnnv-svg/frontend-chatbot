...
import { Download, FileSignature, History, KeyRound, Laptop, Package, Pencil, Plus, RefreshCcw, RotateCcw, Save, Search, Shirt, Smartphone, Trash2, Upload, UserPlus, X } from "lucide-react";
...
import EmployeeAssetHistory from "./EmployeeAssetHistory";
...
export const ASSET_STATUSES = { in_stock: "Trong kho", pending_assignment: "Chờ ký bàn giao", assigned: "Đang cấp", repair: "Bảo hành/Sửa", lost: "Thất lạc", retired: "Ngừng sử dụng" };
...
const ASSET_HOLDER_IMPORT_HEADERS = ["MÃ NHÂN VIÊN ĐANG GIỮ", "NGÀY CẤP PHÁT"];
...
function parseAssetImportRow(row, index) {
  ...
  return {
    ...
    employeeCode: String(cell(row, ["MÃ NHÂN VIÊN ĐANG GIỮ", "MÃ NHÂN VIÊN", "employeeCode"]) || "").trim().toUpperCase(),
    assignedAt: excelIsoDate(cell(row, ["NGÀY CẤP PHÁT", "NGÀY GIAO", "assignedAt"])),
  };
}
...
function statusTone(status) {
  return status === "assigned" ? "bg-blue-100 text-blue-700" : status === "pending_assignment" ? "bg-violet-100 text-violet-700" : status === "in_stock" ? "bg-emerald-100 text-emerald-700" : status === "repair" ? "bg-amber-100 text-amber-700" : "bg-slate-200 text-slate-700";
}

function AssetFormModal({ value, saving, generatingCode, onChange, onClose, onSave }) {
  ...
  return <div className="fixed inset-0 z-[160] flex items-center justify-center bg-slate-950/60 p-3">
    <div className="max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
      ...
      <div className="space-y-5 p-5">
        <section className="rounded-2xl border border-teal-100 bg-teal-50/40 p-4"><h4 className="mb-3 font-black text-teal-800">Nhận dạng thiết bị</h4><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4"><Field label={value._id ? "Mã thiết bị *" : "Mã thiết bị (tự động)"} value={generatingCode && !value._id ? "Đang tạo mã..." : value.assetCode} disabled={!value._id} onChange={(v) => update("assetCode", v)} /><SelectField label="Loại *" value={value.category} onChange={(v) => update("category", v)} options={ASSET_CATEGORIES} /><Field label="Tên thiết bị *" value={value.name} onChange={(v) => update("name", v)} /><Field label="Hãng" value={value.brand} onChange={(v) => update("brand", v)} /><Field label="Model" value={value.model} onChange={(v) => update("model", v)} /><Field label="Serial" value={value.serialNumber} onChange={(v) => update("serialNumber", v)} /><Field label="IMEI" value={value.imei} onChange={(v) => update("imei", v)} /><Field label="Số SIM" value={value.phoneNumber} onChange={(v) => update("phoneNumber", v)} /><Field label="Nhà mạng" value={value.carrier} onChange={(v) => update("carrier", v)} /><SelectField label="Tình trạng" value={value.condition} onChange={(v) => update("condition", v)} options={ASSET_CONDITIONS} /><Field label="Vị trí kho" value={value.warehouseLocation} onChange={(v) => update("warehouseLocation", v)} /><div className="lg:col-span-2"><Field label="Cấu hình/Thông số" value={value.specifications} onChange={(v) => update("specifications", v)} /></div></div></section>
...
function EmployeeDeviceManager({ onClose, standalone = false }) {
  ...
  const [handoverEditor, setHandoverEditor] = useState(null);
  ...
  useEffect(() => {
    if (!editor || editor._id || !editor.category) return undefined;
    let cancelled = false;
    setGeneratingCode(true);
    request(`/api/employee-assets/next-code?category=${encodeURIComponent(editor.category)}`)
      .then((result) => {
        if (cancelled) return;
        setEditor((current) => current && !current._id && current.category === editor.category
          ? { ...current, assetCode: result.data?.assetCode || "" }
          : current);
      })
      .catch((error) => { if (!cancelled) alert(error.message); })
      .finally(() => { if (!cancelled) setGeneratingCode(false); });
    return () => { cancelled = true; };
  }, [editor?._id, editor?.category]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const assignFromWarehouse = async () => {
    ...
    try {
      setSaving(true);
      const result = await request("/api/employee-assets/handovers/prepare", { method: "PATCH", body: JSON.stringify(assignEditor.values) });
      setAssignEditor(null); setHandoverEditor(result.data); await load();
    } catch (error) { alert(error.message); } finally { setSaving(false); }
  };
  const cancelHandover = async () => {
    if (!handoverEditor) return;
    if (!window.confirm(handoverEditor.type === "return" ? "Hủy biên bản thu hồi đang ký? Thiết bị vẫn thuộc nhân viên hiện tại." : "Hủy biên bản đang ký và trả thiết bị về kho?")) return;
    try { setSaving(true); await request(`/api/employee-assets/handovers/${handoverEditor._id}/cancel`, { method: "PATCH" }); setHandoverEditor(null); await load(); }
    catch (error) { alert(error.message); } finally { setSaving(false); }
  };
  const completeHandover = async (signatures) => {
    try { setSaving(true); await request(`/api/employee-assets/handovers/${handoverEditor._id}/complete`, { method: "PATCH", body: JSON.stringify(signatures) }); setHandoverEditor(null); await load(); }
    catch (error) { alert(error.message); } finally { setSaving(false); }
  };
  const resumeHandover = async (asset) => {
    try { const result = await request(`/api/employee-assets/handovers?assetId=${asset._id}`); const draft = (result.data || []).find((item) => ["draft", "processing"].includes(item.status)); if (!draft) throw new Error("Không tìm thấy biên bản đang chờ ký"); setHandoverEditor(draft); }
    catch (error) { alert(error.message); }
  };
  const openHandoverPdf = async (handoverId) => {
    try {
      const response = await fetch(`/api/employee-assets/handovers/${handoverId}/pdf`, { headers: { Authorization: `Bearer ${token}` } });
      if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.message || "Không thể tải biên bản"); }
      const url = URL.createObjectURL(await response.blob()); window.open(url, "_blank", "noopener,noreferrer"); setTimeout(() => URL.revokeObjectURL(url), 60000);
    } catch (error) { alert(error.message); }
  };
  ...
  const downloadImportTemplate = () => {
    const headers = [...ASSET_IMPORT_HEADERS, ...ASSET_HOLDER_IMPORT_HEADERS];
  ...
  return <div className={standalone ? "min-h-full overflow-y-auto bg-slate-50 p-3" : "fixed inset-0 z-[130] overflow-y-auto bg-slate-950/55 p-3 backdrop-blur-sm"}>
    <div className={`mx-auto max-w-[1500px] overflow-hidden rounded-3xl bg-slate-50 shadow-2xl ${standalone ? "min-h-[calc(100vh-48px)]" : "min-h-[calc(100vh-24px)]"}`}>
      ...
      <main className="space-y-4 p-5">
        {importRows.length > 0 && <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><div className="flex flex-wrap items-center gap-3"><div className="mr-auto"><b>Đã đọc {importRows.length} dòng từ {importFileName}</b><p className="text-xs text-slate-600">Nếu có mã nhân viên, thiết bị trong kho sẽ được cấp cho nhân viên tương ứng; thiết bị đã cấp cho người khác sẽ được báo lỗi.</p></div><button onClick={() => setImportRows([])} className="rounded-xl border bg-white px-3 py-2 text-sm">Hủy</button><button disabled={importing} onClick={confirmImport} className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{importing ? "Đang import..." : "Xác nhận import"}</button></div><div className="mt-3 max-h-56 overflow-auto rounded-xl bg-white"><table className="w-full min-w-[800px] text-left text-xs"><thead className="sticky top-0 bg-slate-100"><tr><th className="p-2">Dòng</th><th>Mã thiết bị</th><th>Loại</th><th>Tên thiết bị</th><th>Serial/IMEI/SIM</th><th>Tình trạng</th><th>Mã nhân viên</th></tr></thead><tbody>{importRows.slice(0, 100).map((row) => <tr key={row.rowNumber} className="border-t"><td className="p-2">{row.rowNumber}</td><td className="font-mono">{row.assetCode || <span className="text-red-500">Thiếu</span>}</td><td>{CATEGORY_LABELS[row.category] || <span className="text-red-500">{row.category || "Thiếu"}</span>}</td><td>{row.name || <span className="text-red-500">Thiếu</span>}</td><td>{row.serialNumber || row.imei || row.phoneNumber || "-"}</td><td>{CONDITION_LABELS[row.condition] || row.condition}</td><td className="font-mono">{row.employeeCode || "-"}</td></tr>)}</tbody></table></div>{importRows.length > 100 && <div className="mt-2 text-xs text-slate-500">Đang hiển thị 100/{importRows.length} dòng để xem trước.</div>}</section>}
        {importResult && <section className="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-sm"><div className="flex flex-wrap gap-x-5 gap-y-1"><b>Kết quả import {importResult.total} dòng:</b><span className="text-emerald-700">Tạo mới: {importResult.created || 0}</span><span className="text-blue-700">Cập nhật: {importResult.updated || 0}</span><span className="text-cyan-700">Đã liên kết người giữ: {importResult.assigned || 0}</span><span className="text-slate-600">Không thay đổi: {importResult.unchanged || 0}</span><span className={importResult.errors?.length ? "font-bold text-red-600" : "text-emerald-700"}>Lỗi: {importResult.errors?.length || 0}</span></div>{importResult.errors?.length > 0 && <div className="mt-3 max-h-40 overflow-auto rounded-xl border border-red-100 bg-white p-2 text-xs text-red-700">{importResult.errors.map((error, index) => <div key={index} className="border-b border-red-50 py-1 last:border-0">Dòng {error.row} · {error.assetCode || "Chưa có mã"}: {error.message}</div>)}</div>}</section>}
        <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-7">{[["total", "Tổng"], ["in_stock", "Trong kho"], ["pending_assignment", "Chờ ký"], ["assigned", "Đang cấp"], ["repair", "Sửa chữa"], ["lost", "Thất lạc"], ["retired", "Ngừng dùng"]].map(([key, label]) => <button key={key} onClick={() => key !== "total" && setStatus(key)} className="rounded-2xl border bg-white p-3 text-left shadow-sm"><div className="text-xs font-bold uppercase text-slate-400">{label}</div><div className="mt-1 text-2xl font-black text-slate-800">{summary[key] || 0}</div></button>)}</div>
        ...
        <div className="overflow-auto rounded-2xl border bg-white"><table className="w-full min-w-[1150px] text-left text-sm"><thead className="bg-slate-100 text-xs uppercase text-slate-500"><tr><th className="p-3">Thiết bị</th><th>Mã/Serial</th><th>IMEI/SIM</th><th>Tình trạng</th><th>Trạng thái</th><th>Người đang giữ</th><th>Vị trí/Giá trị</th><th className="pr-3 text-right">Thao tác</th></tr></thead><tbody>{loading ? <tr><td colSpan="8" className="p-10 text-center">Đang tải...</td></tr> : items.length ? items.map((asset) => { const person = asset.currentAssignment?.employeeProfileId; const latestHandoverId = asset.currentAssignment?.handoverId || [...(asset.history || [])].reverse().find((entry) => entry.handoverId)?.handoverId; return <tr key={asset._id} className="border-t hover:bg-teal-50/30"><td className="p-3"><div className="flex items-center gap-2"><span className="text-teal-600">{categoryIcon(asset.category)}</span><div><b>{asset.name}</b><div className="text-xs text-slate-400">{CATEGORY_LABELS[asset.category]} · {asset.brand} {asset.model}</div></div></div></td><td><b className="font-mono">{asset.assetCode}</b><div className="text-xs text-slate-400">{asset.serialNumber || "-"}</div></td><td>{asset.imei || "-"}<div className="text-xs text-slate-400">{asset.phoneNumber || ""} {asset.carrier || ""}</div></td><td>{CONDITION_LABELS[asset.condition] || asset.condition}</td><td>{["assigned", "pending_assignment"].includes(asset.status) ? <span className={`rounded-full px-2 py-1 text-xs font-bold ${statusTone(asset.status)}`}>{ASSET_STATUSES[asset.status]}</span> : <select value={asset.status} onChange={(e) => changeStatus(asset, e.target.value)} className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-bold text-slate-700"><option value="in_stock">Trong kho</option><option value="repair">Bảo hành/Sửa</option><option value="lost">Thất lạc</option><option value="retired">Ngừng sử dụng</option></select>}</td><td>{person ? <><b>{person.personal?.fullName}</b><div className="text-xs text-slate-400">{person.employeeCode} · từ {dateVN(asset.currentAssignment?.assignedAt)}</div></> : "-"}</td><td>{asset.warehouseLocation || "-"}<div className="text-xs text-slate-400">{asset.purchasePrice ? moneyVN(asset.purchasePrice) : ""}</div></td><td className="pr-3 text-right"><div className="flex justify-end gap-1">{asset.status === "pending_assignment" && <button title="Tiếp tục ký biên bản" onClick={() => resumeHandover(asset)} className="flex items-center gap-1 rounded-lg border border-violet-200 bg-violet-50 px-2 py-1.5 text-xs font-bold text-violet-700"><FileSignature size={14} /> Tiếp tục ký</button>}{latestHandoverId && <button title="Xem biên bản gần nhất" onClick={() => openHandoverPdf(latestHandoverId)} className="rounded-lg border border-blue-200 p-2 text-blue-700"><FileSignature size={14} /></button>}{asset.status === "in_stock" && <button title="Cấp phát cho nhân viên" onClick={() => openWarehouseAssign(asset)} className="flex items-center gap-1 rounded-lg border border-blue-200 bg-blue-50 px-2 py-1.5 text-xs font-bold text-blue-700"><UserPlus size={14} /> Cấp phát</button>}<button title="Sửa và xem lịch sử" onClick={() => openEdit(asset)} className="rounded-lg border p-2 text-teal-700"><Pencil size={14} /></button>{!["assigned", "pending_assignment", "in_stock"].includes(asset.status) && <button title="Trả về kho" onClick={() => changeStatus(asset, "in_stock")} className="rounded-lg border p-2 text-emerald-700"><RotateCcw size={14} /></button>}{!["assigned", "pending_assignment"].includes(asset.status) && <button title="Xóa" onClick={() => remove(asset)} className="rounded-lg border p-2 text-red-600"><Trash2 size={14} /></button>}</div></td></tr>; }) : <tr><td colSpan="8" className="p-10 text-center text-slate-500">Chưa có thiết bị phù hợp.</td></tr>}</tbody></table></div>
      </main>
    </div>
    {editor && <AssetFormModal value={editor} saving={saving} generatingCode={generatingCode} onChange={setEditor} onClose={() => setEditor(null)} onSave={save} />}
    ...
    {handoverEditor && <AssetHandoverModal handover={handoverEditor} saving={saving} onCancel={cancelHandover} onComplete={completeHandover} />}
  </div>;
}
...
export function EmployeeAssetSection({ profile, onChanged }) {
  ...
  const assign = async () => {
    ...
    try { setSaving(true); const result = await request("/api/employee-assets/handovers/prepare", { method: "PATCH", body: JSON.stringify({ ...assignEditor, profileId: profile._id }) }); setAssignEditor(null); setHandoverEditor(result.data); await load(); }
    catch (error) { alert(error.message); } finally { setSaving(false); }
  };
  const cancelHandover = async () => {
    if (!window.confirm(handoverEditor?.type === "return" ? "Hủy biên bản thu hồi đang ký? Thiết bị vẫn thuộc nhân viên hiện tại." : "Hủy biên bản đang ký và trả thiết bị về kho?")) return;
  ...
  const completeHandover = async (signatures) => {
    try { setSaving(true); await request(`/api/employee-assets/handovers/${handoverEditor._id}/complete`, { method: "PATCH", body: JSON.stringify(signatures) }); setHandoverEditor(null); setHistoryRevision((value) => value + 1); await load(); await onChanged?.(); }
    catch (error) { alert(error.message); } finally { setSaving(false); }
  };
  const openHandoverPdf = async (handoverId) => {
    try { const response = await fetch(`/api/employee-assets/handovers/${handoverId}/pdf`, { headers: { Authorization: `Bearer ${token}` } }); if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.message || "Không thể tải biên bản"); } const url = URL.createObjectURL(await response.blob()); window.open(url, "_blank", "noopener,noreferrer"); setTimeout(() => URL.revokeObjectURL(url), 60000); }
    catch (error) { alert(error.message); }
  };
  const openReturn = async (asset) => {
    try {
      setSaving(true);
      const result = await request(`/api/employee-assets/handovers?assetId=${asset._id}`);
      const pendingReturn = (result.data || []).find((item) => item.type === "return" && ["draft", "processing"].includes(item.status));
      if (pendingReturn) setHandoverEditor(pendingReturn);
      else setReturnEditor({ asset, values: { ...emptyReturn, condition: asset.condition || "good" } });
    } catch (error) { alert(error.message); } finally { setSaving(false); }
  };
  const doReturn = async () => {
    try { setSaving(true); const result = await request("/api/employee-assets/handovers/returns/prepare", { method: "PATCH", body: JSON.stringify({ assetId: returnEditor.asset._id, ...returnEditor.values }) }); setReturnEditor(null); setHandoverEditor(result.data); }
    catch (error) { alert(error.message); } finally { setSaving(false); }
  };
