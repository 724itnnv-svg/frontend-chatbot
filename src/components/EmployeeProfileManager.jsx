...
import { AlertTriangle, ArrowLeft, BadgeCheck, Building2, CalendarClock, Download, FileText, History, Plus, RefreshCcw, Save, Search, Upload, UserRound, X } from "lucide-react";
...
function TextAreaField({ label, value, onChange, rows = 5 }) {
  return <label className="block"><span className={labelClass}>{label}</span><textarea rows={rows} value={value ?? ""} onChange={(e) => onChange(e.target.value)} className={`${inputClass} resize-y font-mono leading-relaxed`} /></label>;
}
  ...
  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/60 p-3 backdrop-blur-sm">
    <div className="flex max-h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto bg-slate-50 p-5">
        ...
        <section className="rounded-2xl border border-violet-100 bg-white p-4"><div className="mb-3"><h4 className="font-black text-violet-800">Nội dung hợp đồng</h4><p className="mt-1 text-xs text-slate-500">Mỗi dòng là một đoạn. Biến hỗ trợ: {"{{contractType}}"}, {"{{duration}}"}, {"{{effectiveDate}}"}, {"{{expiryDate}}"}, {"{{workplace}}"}, {"{{companyAddress}}"}, {"{{department}}"}, {"{{jobTitle}}"}, {"{{jobDescription}}"}, {"{{workingHours}}"}, {"{{salary}}"}, {"{{allowances}}"}, {"{{paymentMethod}}"}.</p></div><div className="grid gap-4 lg:grid-cols-2">{CONTRACT_CONTENT_FIELDS.map(([key, label]) => <TextAreaField key={key} label={label} value={settings.content?.[key]} onChange={(v) => updateGroup("content", key, v)} />)}</div></section>
      </div>
...
export default function EmployeeProfileManager({ users, onClose, standalone = false }) {
  ...
  useEffect(() => { loadProfiles(); loadAlerts(); loadDocumentDefaults(); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const openBulkDocumentSettings = () => {
    setShowDocumentPicker(false);
    setDocumentEditor({ bulk: true, settings: mergeDocumentSettings(documentDefaults) });
  };
  ...
  const saveDocumentSettings = async () => {
    try {
      ...
      if (documentEditor.bulk) {
        const result = await request("/api/employee-profiles/contract-document-settings/bulk", { method: "PUT", body: JSON.stringify({ documentSettings: documentEditor.settings }) });
        setDocumentEditor(null);
        alert(`Đã áp dụng cho ${result.data?.matched || 0} hợp đồng của nhân viên`);
        return;
      }
  ...
  const content = <div className={standalone ? "min-h-full overflow-y-auto bg-slate-50 p-3" : "fixed inset-0 z-[100] overflow-y-auto bg-slate-950/45 p-3 backdrop-blur-sm"}>
    <div className={`mx-auto max-w-[1500px] rounded-3xl border border-cyan-100 bg-gradient-to-b from-cyan-50 to-white shadow-2xl ${standalone ? "min-h-[calc(100vh-48px)]" : "min-h-[calc(100vh-24px)]"}`}>
      ...
      {!editor ? <main className="p-5">
        ...
        {editor._id && <section className="rounded-2xl border border-cyan-100 bg-white p-4"><div className="mb-4 flex items-center"><div className="mr-auto"><h3 className="font-black text-cyan-800">Hợp đồng và phụ lục</h3><p className="text-xs text-slate-500">{editor.contracts?.length || 0} hợp đồng</p></div>{canProfileAction("edit") && <button onClick={() => setContractEditor(clone(emptyContract))} className="flex items-center gap-2 rounded-xl bg-cyan-600 px-3 py-2 text-sm font-bold text-white"><Plus size={15} /> Thêm hợp đồng</button>}</div><div className="grid gap-3 md:grid-cols-2">{editor.contracts?.map((c) => <div key={c._id} className="rounded-xl border border-cyan-100 p-3"><div className="flex gap-2"><FileText className="text-cyan-600" size={20} /><div className="mr-auto"><b>{c.contractNumber}</b><div className="text-xs text-slate-500">{dateInput(c.effectiveDate)} → {c.contractType === "indefinite" ? "Không thời hạn" : dateInput(c.expiryDate)}</div></div>{canProfileAction("edit") && <button onClick={() => setContractEditor({ ...clone(emptyContract), ...c, signedDate: dateInput(c.signedDate), effectiveDate: dateInput(c.effectiveDate), expiryDate: dateInput(c.expiryDate), renewalDueDate: dateInput(c.renewalDueDate), companyRepresentative: { ...emptyContract.companyRepresentative, ...c.companyRepresentative }, appendices: (c.appendices || []).map((a) => ({ ...a, signedDate: dateInput(a.signedDate), effectiveDate: dateInput(a.effectiveDate), expiryDate: dateInput(a.expiryDate) })) })} className="rounded-lg border px-2 text-xs">Sửa</button>}</div><div className="mt-3 flex gap-2">{canProfileAction("export") && <><button onClick={() => exportContract(c, "docx")} className="rounded-lg bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700">Word</button><button onClick={() => exportContract(c, "pdf")} className="rounded-lg bg-red-50 px-2 py-1 text-xs font-bold text-red-700">PDF</button></>}<span className="ml-auto text-xs text-slate-400">{c.appendices?.length || 0} phụ lục</span></div></div>)}</div></section>}
        {editor._id && editor.contracts?.length > 0 && <section className="rounded-2xl border border-violet-100 bg-violet-50/60 p-4"><div className="mb-3"><h3 className="font-black text-violet-800">Nội dung và định dạng hợp đồng</h3><p className="text-xs text-slate-500">Chỉnh riêng nội dung, font chữ và định dạng cho từng hợp đồng của nhân viên này.</p></div><div className="flex flex-wrap gap-2">{editor.contracts.map((contract) => <button key={contract._id} onClick={() => openDocumentSettings(contract)} className="flex items-center gap-2 rounded-xl border border-violet-200 bg-white px-3 py-2 text-sm font-bold text-violet-700 shadow-sm transition hover:border-violet-400 hover:bg-violet-100"><FileText size={15} /> Sửa {contract.contractNumber}</button>)}</div></section>}
    ...
    {showDocumentPicker && <div className="fixed inset-0 z-[115] flex items-center justify-center bg-slate-950/55 p-4">
      <div className="flex max-h-[88vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="border-b border-violet-100 bg-violet-50 p-4"><button onClick={openBulkDocumentSettings} className="flex w-full items-center gap-3 rounded-xl border border-violet-200 bg-white p-3 text-left shadow-sm transition hover:border-violet-400 hover:bg-violet-50"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-violet-600 text-white"><UserRound size={19} /></span><div className="mr-auto"><div className="text-sm font-black text-violet-800">Cập nhật cho tất cả nhân viên</div><div className="text-xs text-slate-500">Áp dụng cùng nội dung, font chữ và định dạng cho toàn bộ hợp đồng hiện có</div></div><span className="rounded-lg bg-violet-100 px-3 py-1 text-xs font-bold text-violet-700">Chọn</span></button></div>
